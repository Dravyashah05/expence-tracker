        ...
        @if (showForm()) {
          <div class="goal-form-card glass-card">
            <div class="form-head">
              ...
              <button class="close-form-btn" type="button" (click)="closeForm()">
            ...
            <div class="form-row">
              ...
              <div class="fg">
                ...
                <div class="limit-input-wrap">
                  ...
                  <input
                    class="styled-input limit-input"
                    type="number"
                    [(ngModel)]="formLimit"
                    step="1"
                    min="1"
          ...
          <div class="goals-grid">
            @for (goal of goalDetails(); track goal.id) {
              <div
                ...
                [class.over-budget]="goal.isOver"
                [class.near-limit]="goal.isNear && !goal.isOver"
              >
                <!-- Card header -->
                <div class="goal-card-head">
                  <div
                    class="goal-icon-wrap"
                    [class.icon-over]="goal.isOver"
                    [class.icon-near]="goal.isNear && !goal.isOver"
                  >
                ...
                <div class="goal-amounts">
                  <div class="goal-spent">
                    ...
                    <strong class="amt-val" [class.over]="goal.isOver">{{
                      goal.spent | appCurrency
                    }}</strong>
                  </div>
                  ...
                  <div class="goal-remaining">
                    ...
                    <strong
                      class="amt-val"
                      [class.positive]="!goal.isOver"
                      [class.over]="goal.isOver"
                    >
                      {{
                        goal.isOver
                          ? (goal.spent - goal.limit | appCurrency)
                          : (goal.limit - goal.spent | appCurrency)
                      }}
                    </strong>
                  </div>
                  <div class="goal-divider"></div>
                ...
                <div class="progress-wrap">
                  ...
                  <div class="progress-labels">
                    ...
                    @if (goal.isOver) {
                      <span class="status-tag danger-tag"
                        ><mat-icon>warning</mat-icon> Over Budget</span
                      >
                    } @else if (goal.isNear) {
                      <span class="status-tag warn-tag"><mat-icon>alarm</mat-icon> Near Limit</span>
                    } @else {
                      <span class="status-tag ok-tag"
                        ><mat-icon>check_circle</mat-icon> On Track</span
                      >
                    }
                  </div>
                </div>
                ...
                <p class="goal-context">
                  Day {{ todayDay() }} of {{ daysInMonth() }} · {{ daysLeft() }} days remaining this
                  month
                </p>
              </div>
            }
    ...
    `
      ...
      .fill-near {
        background: linear-gradient(
          90deg,
          var(--accent),
          color-mix(in srgb, var(--accent) 80%, #f00)
        );
      }
      .fill-over {
        background: linear-gradient(
          90deg,
          var(--danger),
          color-mix(in srgb, var(--danger) 80%, #000)
        );
      }
...
export class BudgetGoalsComponent implements OnInit {
  ...
  daysInMonth = computed(() =>
    new Date(this.now.getFullYear(), this.now.getMonth() + 1, 0).getDate(),
  );
  ...
  ngOnInit(): void {
    ...
    if (stored) {
      try {
        this.goals.set(JSON.parse(stored));
      } catch {
        this.goals.set([]);
      }
    }
  ...
  saveGoal(): void {
    ...
    if (!this.formLimit || this.formLimit <= 0) {
      this.formError.set('Please enter a valid budget limit.');
      return;
    }
    ...
    if (editId) {
      this.goals.update((g) =>
        g.map((goal) =>
          goal.id === editId
            ? { ...goal, category: this.formCategory, icon, monthlyLimit: Number(this.formLimit) }
            : goal,
        ),
      );
    } else {
      ...
      if (exists) {
        this.formError.set('A budget goal for this category already exists.');
        return;
      }
